...
namespace SWSDK.Services
{
        ...
        where T : Response, new()
    {
        ...
        public virtual async Task<T> GetDeleteResponseAsync(string url, string path, Dictionary<string, string> headers, HttpClientHandler proxy)
        {
            try
            {
                using (HttpClient client = new HttpClient(proxy))
